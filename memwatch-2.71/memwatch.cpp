...
//Undef stuff we redefined, so it will not be recursive (precompiled headers seem to cause recursion otherwise)
...
#if defined(MW_PTHREADS) || defined(HAVE_PTHREAD_H)
#error "Whould not get here with WIN32 and no PTHREAD"
...
static void mwLogFile( const char *name ) {
#ifndef ENABLEMWFILEWRITES
//We are using cprintf, so there iw no need to have any files opened.
	mwLogW(NULL);
#else
    time_t tid;
    (void) time( &tid );
...
SECURITY_ATTRIBUTES MutexAttributes;
	...
	MutexAttributes.nLength=sizeof(SECURITY_ATTRIBUTES);
	MutexAttributes.lpSecurityDescriptor=NULL;		//go with defaults
	MutexAttributes.bInheritHandle=true;				//allow child processes inherit the handle

	mwGlobalMutex = CreateMutex(&MutexAttributes, FALSE, "memwatch 2.71");	//cant initialize with no ecurity descriptors, as child proceses can not use it otherwize
	return;
}
